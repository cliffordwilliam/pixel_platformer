    ...
    def input(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_SPACE:
                # Pre process
                for group in self.room_groups_list:
                    for room_index, room in enumerate(group):
                        for cell_index, cell in enumerate(room):
                            if cell != 0:
                                group[room_index][cell_index] = {
                                    "name": cell.name,
                                    "position_x": cell.position.x,
                                    "position_y": cell.position.y,
                                    "frame_index": cell.frame_index,
                                }
                # Serialize the modified list to JSON and save it to a file
                with open("rooms/new_room.json", "w") as f:
                    json.dump(self.room_groups_list, f, indent=4)
